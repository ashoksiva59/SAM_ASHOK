...
export class TimeSheetEventLibrary {
    ...
    static TimeSheetEntryDetailsOnPageLoad(clientAPI) {
        ...
        let relativeDayOfWeek = libCom.relativeDayOfWeek(odataDate.date(), clientAPI);

        clientAPI.setCaption(relativeDayOfWeek + ', ' + clientAPI.formatDate(odataDate.date()));
    ...
    static ValidateFutureDate(pageClientAPI, dict) {
        ...
        if (currentDate > dict.HourStartDtPicker && currentDate > dict.HourEndDtPicker) {
            ...
            let message = pageClientAPI.localizeText('validation_start_time_cannot_be_in_the_future');
            return libCom.showErrorDialog(pageClientAPI, message);
        } else if (currentDate < dict.HourEndDtPicker) {
            let message = pageClientAPI.localizeText('validation_finish_time_in_the_future');
            return libCom.showErrorDialog(pageClientAPI, message);
        }
        return Promise.resolve(true);
    }
    ...
    static ValidateStartBeforeFinish(pageClientAPI, dict) {
        if (dict.HourEndDtPicker > dict.HourStartDtPicker) {
            ...
            let message = pageClientAPI.localizeText('validation_start_time_must_be_before_finish_time');
            return libCom.showErrorDialog(pageClientAPI, message);
        }
    ...
    static ValidateHoursNonZero(pageClientAPI, dict) {
        if (dict.DurationPkr > 0) {
            ...
            let message = pageClientAPI.localizeText('validation_non_zero_hours');
            return libCom.showErrorDialog(pageClientAPI, message);
        }
    ...
    static ValidateActivityTypeIfRecOrder(pageClientAPI, dict) {
        if (!libVal.evalIsEmpty(dict.RecOrderLstPkr)) {
            if (!libVal.evalIsEmpty(dict.ActivityTypeLstPkr)) {
                ...
                let message = pageClientAPI.localizeText('validation_activity_type_is_required_workorder');
                return libCom.showErrorDialog(pageClientAPI, message);
            }
        } else {
            return Promise.resolve(true);
    ...
    static ValidateOperationIfRecOrder(pageClientAPI, dict) {
        if (!libVal.evalIsEmpty(dict.RecOrderLstPkr)) {
            if (!libVal.evalIsEmpty(dict.OperationLstPkr)) {
                ...
                let message = pageClientAPI.localizeText('validation_operation_is_required_if_workorder');
                return libCom.showErrorDialog(pageClientAPI, message);
            }
        } else {
            return Promise.resolve(true);
    ...
    static ValidateWorkorderRequiredIfActivityType(pageClientAPI, dict) {
        if (!libVal.evalIsEmpty(dict.ActivityTypeLstPkr)) {
            if (!libVal.evalIsEmpty(dict.RecOrderLstPkr)) {
                ...
                let message = pageClientAPI.localizeText('validation_workorder_required_if_activity_type_specified');
                return libCom.showErrorDialog(pageClientAPI, message);
            }
        } else {
            return Promise.resolve(true);
    ...
    static ValidateAbsAttOrActivityType(pageClientAPI, dict) {
        if (!libVal.evalIsEmpty(dict.ActivityTypeLstPkr) || !libVal.evalIsEmpty(dict.AbsAttLstPkr)) {
            ...
            let message = pageClientAPI.localizeText('validation_absAtt_or_activity_type');
            return libCom.showErrorDialog(pageClientAPI, message);
        }
    }
        ...
        return totalHours.then(function(value) {
            ...
            if (onCreate && (parseFloat(value) + currentHours) <= 24.0 ) {
                ...
                if (pageClientAPI.getClientData().EmployeeName) {
                    message = pageClientAPI.localizeText('validation_daily_total_exceeds_24_hours_for');
                    message += ` ${pageClientAPI.getClientData().EmployeeName}`;
                } else {
                    message = pageClientAPI.localizeText('validation_daily_total_exceeds_24_hours');
                }
                return libCom.showErrorDialog(pageClientAPI, message);
            }
        });
...
export class TimeSheetLibrary {
    ...
    static GetWorkCenterFromRecOrder(pageProxy, readLink) {
        return pageProxy.read('/SAPAssetManager/Services/AssetManager.service', readLink, [], '')
            .then(function(workorders) {
                var record = workorders.getItem(0).MainWorkCenter;
                if (record) {
                    return record;
                } else {
                    return undefined;
    ...
    static ShouldEnableSubOperations(pageProxy) {
        ...
        return pageProxy.read('/SAPAssetManager/Services/AssetManager.service', operation + '/SubOperations', [], '')
            .then(function(subOps) {
    ...
    static updateWorkCenter(context, workCenter) {
        ...
        function updateActivityType() {
            return context.read('/SAPAssetManager/Services/AssetManager.service', 'WorkCenters', [], `$filter=ExternalWorkCenterId eq '${workCenter}'`).then(result => {
                if (result && result.getItem(0)) {
                    ...
                    activitySpecifier.setQueryOptions(`$filter=CostCenter eq '${costCenter}'&orderby=ActivityType`);
                    activityListPickerProxy.setTargetSpecifier(activitySpecifier);
                }
            });
        }

        let workCenterProxy = libCom.getControlProxy(context, 'WorkCenterLstPkr');
        workCenterProxy.setValue(workCenter);
        return updateActivityType(context, workCenter);
    }
}
