...
export default class {
                    ...
                    doMarkComplete => {
                        ...
                        return parent.showTimeCaptureMessage(pageContext, true).then(() => {
                            if (action.didExecute) {
                                // Action executed, update UI accordingly
    ...
    static operationRollUpMobileStatus(context, entitySet) {
        ...
        if (!isLocal) {
            ...
            return context.read('/SAPAssetManager/Services/AssetManager.service', entitySet, [], "$filter=(OrderId eq '" + orderID + "')")
                .then(function(results) {
                    if (results) {
                        var oprCount = results.length;
                        if (oprCount > 0) {
                            var oprStartQueryOption = '$filter=';
                            var oprHoldQueryOption = '$filter=';
                            var oprCompleteQueryOption = '$filter=';
