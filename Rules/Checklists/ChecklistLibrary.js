   ...
   /**
    ...
    static allowWorkOrderComplete(context, equipment) {
        ...
        return context.count('/SAPAssetManager/Services/AssetManager.service',"MyEquipments('" + equipment + "')/EquipAssessments_Nav", "$expand=Form_Nav&$filter=Form_Nav/MobileStatus ne '" + completed + "'").then(count => {
            ...
            let defaultChecklistMessage = context.localizeText('default_checklist_message');
            let confirmChecklist = context.localizeText('confirm_checklist');
            return libCom.showWarningDialog(context, defaultChecklistMessage, confirmChecklist).then(successResult => {
