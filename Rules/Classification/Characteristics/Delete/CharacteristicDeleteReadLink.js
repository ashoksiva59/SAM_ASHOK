...
export default function CharacteristicDeleteReadLink(context) {
    ...
    if (isMultiListPicker && !isLocal) {
        return entitySet + '(CharId='+ '\'' + context.binding.CharId + '\',' + 'CharValCounter=\'' + charValCounter +'\','+ 'ClassType=\'' + context.evaluateTargetPathForAPI('#Page:-Previous').binding.ClassType + '\',' + 'InternCounter=\'0000\',ObjClassFlag=' + '\'' + context.evaluateTargetPathForAPI('#Page:-Previous').binding.ObjClassFlag + '\''+ ',ObjectKey='+ '\'' +context.evaluateTargetPathForAPI('#Page:-Previous').binding.ObjectKey + '\''+ ')&$orderby=CharId';
