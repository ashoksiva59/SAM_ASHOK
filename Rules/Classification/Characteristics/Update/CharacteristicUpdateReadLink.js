...
import charValCounter from '../../Characteristics/CharValCounter';
...
export default function CharacteristicUpdateReadLink(context) {
    ...
    if (isMultiListPicker) {
        return entitySet + '(CharId='+ '\'' + context.binding.CharId + '\',' + 'CharValCounter=\'' + charValCounter(context) +'\','+ 'ClassType=\'' + context.evaluateTargetPathForAPI('#Page:-Previous').binding.ClassType + '\',' + 'InternCounter=\'0000\',ObjClassFlag=' + '\'' + context.evaluateTargetPathForAPI('#Page:-Previous').binding.ObjClassFlag + '\''+ ',ObjectKey='+ '\'' +context.evaluateTargetPathForAPI('#Page:-Previous').binding.ObjectKey + '\''+ ')&$orderby=CharId';
    } else {
