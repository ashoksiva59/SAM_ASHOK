...
export default class {
    ...
    static taskNotCompleted(context) {
        return this.showNotificationPendingTaskMessage(context);
    }
    ...
    static showNotificationPendingTaskMessage(context) {
        let message = context.localizeText('notification_pending_task_warning_text');
        return libMobile.showErrorDialog(context, message);
    }
    ...
    static getHeaderMobileStatus(context) {
        try {
            let pageProxy = context.evaluateTargetPathForAPI('#Page:' + notificationDetailsPage);
            return new Promise((resolve, reject) => {
                try {
                    libMobile.mobileStatus(pageProxy, pageProxy.binding).then(status => {
                        resolve(status);
                    });
                } catch (error) {
                    reject('');
