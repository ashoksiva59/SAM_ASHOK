...
export default function CharacteristicMultiValues(context) {
    ...
    if (controlName ==='CurrencyMultipleValue' || controlName ==='CharacterMultipleValue' || controlName ==='NumberMultipleValue') {
        ...
        return context.read('/SAPAssetManager/Services/AssetManager.service', entityType, [], '$filter=(CharId eq \'' + context.binding.CharId + '\' and '+ idType +' eq \'' + context.binding.id + '\')&$orderby=CharId').then(function(results) {
            for (var i = 0; i < results.length; i++) {
                ...
                if (controlName === 'CharacterMultipleValue') {
                    ...
                    jsonResult.push(
                        {
                            'DisplayValue': assembleDisplayValues(obj, obj.ValueRel,  obj.CharValFrom.toString(), obj.CharValTo.toString(), 'MultipleValues'),
            ...
            return context.read('/SAPAssetManager/Services/AssetManager.service', 'ClassCharacteristicValues', [], '$filter=(CharId eq \'' + context.binding.CharId + '\'' + queryOptions + ')').then(function(result) {
                for (var k = 0; k < result.length; k++) {
                    ...
                    if (controlName === 'CharacterMultipleValue') {
                        ...
                        jsonResult.push(
                            {
                                'DisplayValue': assembleDisplayValues(context, resultObj.ValueRel, resultObj.CharValFrom.toString(), resultObj.CharValTo.toString()),
